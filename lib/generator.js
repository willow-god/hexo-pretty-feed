...
function getXslContent(type, feedConfig) {
  const xslPath = feedConfig[`pretty_${type}_file`];
  if (!xslPath) return null;

  if (xslCache.has(xslPath)) {
    return xslCache.get(xslPath);
  }

  try {
    const content = readFileSync(xslPath, 'utf8');
    xslCache.set(xslPath, content);
    return content;
  } catch (err) {
    xslCache.set(xslPath, null);
    return null;
  }
}

function injectXsl(xml, xslContent) {
  if (!xslContent) return xml;
  // 在 XML 声明后插入 XSL 处理指令
  const xslInstruction = `<?xml-stylesheet type="text/xsl" href="data:text/xsl;base64,${Buffer.from(xslContent).toString('base64')}"?>`;
  return xml.replace(/(<\?xml[^?]*\?>)/, `$1\n${xslInstruction}`);
}

function generateRss(feed, items, feedConfig) {
  ...
  const xslContent = getXslContent('rss2', feedConfig);
  xml = injectXsl(xml, xslContent);

  return xml;
}

function generateAtom(feed, items, feedConfig) {
  const links = [
  ...
  const xslContent = getXslContent('atom', feedConfig);
  xml = injectXsl(xml, xslContent);
...
module.exports = function(locals, type, path) {
  ...
  switch (type) {
    case 'rss2':
      data = generateRss(feed, items, feedConfig);
      break;
    default:
      data = generateAtom(feed, items, feedConfig);
  }
