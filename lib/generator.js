...
function composeItemContent(post, feedConfig, context) {
  const { content } = feedConfig;

  if (content && post.content) {
    const cleanContent = post.content.replace(/[\x00-\x1F\x7F]/g, '');

    const imageUrl = post.image || post.cover;

    // 避免重复：如果正文已经有 img，就不再插入封面
...
function composeItem(post, feedConfig, context) {
  ...
  return {
    ...
    content: composeItemContent(post, feedConfig, context),
